...
const NewestWatchSection = (): JSX.Element => {
  ...
  useGSAP(() => {
    ...
    .to(img, {height: 336, duration: 1, ease: "none"})
    .to(img, {height: 915, duration: 1, ease: "none"})
    .to(img, {opacity: 0, duration: 1, ease: "none"})
    .fromTo(div, {paddingTop: 93, paddingBottom: 93}, {paddingTop: 0, paddingBottom: 0, ease: "none"})
    .to(background, {opacity: 1, duration: 1})
    .to(price, {opacity: 1, duration: 1, ease: "none"})
  ...
  return (
    <div ref={sectRef} className='h-278.75 max-[1024px]:max-w-screen flex items-start relative flex-col justify-between bg-white w-full z-10'>
      ...
      <div className={`max-[1024px]:px-26 max-[740px]:px-14 max-[573px]:px-10 max-[541px]:px-8 py-25 px-50 flex flex-col justify-between h-full w-full`}>
        ...
        <div className='flex flex-col items-start max-[923px]:text-center max-[923px]:w-full max-[666px]:gap-1 max-[495px]:gap-1'>
          ...
          <h1 ref={h1Ref} className='text-[4rem] uppercase mix-blend-difference text-white text-left max-w-260 w-full font-semibold not-italic max-[923px]:w-full max-[819px]:text-[3rem] max-[666px]:text-[2.5rem] max-[819px]:leading-12 max-[446px]:text-[2rem]'>The Brantium Grand <br/> Complication No. 1</h1>

          <p ref={pFirstRef} className='text-[1rem] mix-blend-difference text-white max-w-150 self-stretch font-light not-italic leading-[1.3rem] tracking-[-0.02rem] max-[923px]:w-full max-[923px]:text-lg max-[923px]:leading-6 max-[819px]:text-[1rem] max-[495px]:leading-5 max-[446px]:text-[0.9rem]'>An ultra-exclusive mechanical masterpiece engineered for elite collectors and long-term investors. Hand-assembled, individually numbered, and produced in strictly limited quantities.</p>
      ...
      <div ref={animatedDiv} className='bg-black z-[-1] absolute self-center py-6'>
        <div ref={innerAnimatedDiv} className='h-full relative flex items-center justify-center'>
          ...
          <div
            ...
            className='absolute h-full w-full opacity-1 inset-0 bg-no-repeat'
            style={{background: `linear-gradient(0deg, rgba(0, 0, 0, 0.40) 0%, rgba(0, 0, 0, 0.20) 100%), url(${Bg}) lightgray 50% / cover no-repeat`}}
          />
